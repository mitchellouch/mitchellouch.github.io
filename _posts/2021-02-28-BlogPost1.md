...
Here, we are passing the port and host (localhost:3000) into the .listen function which is asynchronously listening for http requests on the specified port. Now, what about this funny looking third argument being passed: `() => {}` This one of a few ways to use a callback function, you can write anything you want to happen when a request is processed between the {}.

###### 3.	Promises
A Promise in JavaScript is also exactly what it sounds like: we're making a promise to do a certain action which is either resolved or rejected.
